    ...
    projects: {
      ...
      items: {
        ...
        fidalgoselect: {
          ...
          longDescription: "Two applications built end to end for AF Select, a curated marketplace for high-end vehicles and properties across Tucumán, Salta and Buenos Aires. The public site is a React 18 + Vite SPA in JavaScript with hand-written CSS, backed by Supabase: filterable catalog, listing detail pages, map views with Leaflet, a sell-your-asset form, testimonials, and inquiry flows that open WhatsApp with the message already composed from the listing's title, price and link. Full technical SEO — dynamic meta tags, JSON-LD structured data, robots.txt, sitemap and Search Console verification. The second application is a private CRM (React + Tailwind CSS v4) the client runs themselves: a listings manager with image upload and deletion, a section manager where each catalog section defines its own configurable fields, hero management, a leads inbox, and a dashboard with catalog valuation KPIs split by USD and ARS using es-AR price formatting. The client publishes, edits and prices everything without touching code."
        },
        costear: {
          title: "CosteAR",
    ...
    projects: {
      ...
      items: {
        ...
        fidalgoselect: {
          ...
          longDescription: "Dos aplicaciones desarrolladas de punta a punta para AF Select, un marketplace curado de vehículos de alta gama y propiedades en Tucumán, Salta y Buenos Aires. El sitio público es una SPA React 18 + Vite en JavaScript con CSS propio, respaldada por Supabase: catálogo con filtros, fichas de detalle, vistas de mapa con Leaflet, formulario para vender, testimonios y flujos de consulta que abren WhatsApp con el mensaje ya redactado a partir del título, el precio y el link de la publicación. SEO técnico completo — meta tags dinámicos, datos estructurados JSON-LD, robots.txt, sitemap y verificación en Search Console. La segunda aplicación es un CRM privado (React + Tailwind CSS v4) que opera el propio cliente: gestor de publicaciones con carga y borrado de imágenes, administrador de secciones donde cada sección del catálogo define sus propios campos configurables, gestión del hero, bandeja de leads y un dashboard con KPIs de valuación del catálogo separados en USD y ARS con formato de precios es-AR. El cliente publica, edita y cotiza todo sin tocar código."
        },
        portfolio: {
